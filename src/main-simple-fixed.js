...
let authToken = null // FÃ¼r spÃ¤ter wenn wir JWT implementieren
...
async function saveCharacterToAccount(character) {
  // Nur Felder senden, die im Backend existieren
  const updateData = {
...
async function saveCharacterToBackend(character) {
  // Hilfsfunktion fÃ¼r Kampf-Ende und andere Updates
  ...
  try {
    ...
    const updateData = {
      ...
      equipped_accessory: character.equipped_accessory
    }
    const updated = await apiRequest(`/characters/${character.id}`, 'PUT', updateData)
    return updated
  } catch (error) {
...
// ===== Item-System =====
const RARITIES = {
  Common: { mult: 1, color: '#ffffff', name: 'GewÃ¶hnlich', prefix: ['Einfacher', 'Gemeiner', 'Schlichter'] },
  Rare: { mult: 1.5, color: '#0088ff', name: 'Selten', prefix: ['Spezieller', 'Seltener', 'Bemerkenswerter'] },
  Epic: { mult: 2, color: '#aa00ff', name: 'Episch', prefix: ['Epischer', 'LegendÃ¤rer', 'Mythischer'] },
  Legendary: { mult: 3, color: '#ffaa00', name: 'LegendÃ¤r', prefix: ['LegendÃ¤rer', 'GÃ¶ttlicher', 'Uralter'] }
}
...
const ITEM_TYPES = {
  ...
  fist: { name: 'FÃ¤ustlinge', slot: 'weapon', stat: 'dexterity', offhand: 'prayer' },
  shield: { name: 'Schild', slot: 'offhand', stat: 'strength' },
  tome: { name: 'Zauberbuch', slot: 'offhand', stat: 'intelligence' },
  quiver: { name: 'KÃ¶cher', slot: 'offhand', stat: 'dexterity' },
  ...
  armor: { name: 'RÃ¼stung', slot: 'armor', stat: 'vitality' },
...
const CLASS_WEAPONS = {
  ...
  'SchÃ¼tze': { weapon: 'twbow', offhand: null },
...
function genItem(level, type, rarity, playerClass) {
  ...
  return {
    ...
    color: rd.color,
    description: td.slot === 'weapon' ? 'Waffe' : td.slot === 'offhand' ? 'Nebenhand' : td.slot === 'armor' ? 'RÃ¼stung' : 'Accessoire'
  }
}

// ===== Classes =====
const CLASSES = [
  { name: 'Krieger', desc: 'StÃ¤rke', stats: ['StÃ¤rke'] },
  { name: 'Magier', desc: 'Intelligenz', stats: ['Intelligenz'] },
  { name: 'SchÃ¼tze', desc: 'Geschicklichkeit', stats: ['Geschicklichkeit'] },
  { name: 'Priester', desc: 'Weisheit', stats: ['Weisheit'] },
  { name: 'Paladin', desc: 'VitalitÃ¤t', stats: ['VitalitÃ¤t'] },
  { name: 'Battlemage', desc: 'StÃ¤rke+Intelligenz', stats: ['StÃ¤rke','Intelligenz'] },
  { name: 'Ranger', desc: 'StÃ¤rke+Geschick', stats: ['StÃ¤rke','Geschicklichkeit'] },
  { name: 'Cleric', desc: 'StÃ¤rke+Weisheit', stats: ['StÃ¤rke','Weisheit'] },
  { name: 'Juggernaut', desc: 'StÃ¤rke+VitalitÃ¤t', stats: ['StÃ¤rke','VitalitÃ¤t'] },
  ...
  { name: 'Warlock', desc: 'Int+VitalitÃ¤t', stats: ['Intelligenz','VitalitÃ¤t'] },
  { name: 'Monk', desc: 'Geschick+Weisheit', stats: ['Geschicklichkeit','Weisheit'] },
  { name: 'Evasion Tank', desc: 'Geschick+VitalitÃ¤t', stats: ['Geschicklichkeit','VitalitÃ¤t'] },
  { name: 'Healer-Tank', desc: 'Weisheit+VitalitÃ¤t', stats: ['Weisheit','VitalitÃ¤t'] }
]
...
class GameLoop {
  ...
  recalc() {
    ...
    if (this.player.equipment) {
      Object.values(this.player.equipment).forEach(item => {
        if (item.stat === 'strength') this.player.armor += item.statValue * 2
        if (item.stat === 'vitality') this.player.max_hp += item.statValue * 10
        if (item.stat === 'dexterity') this.player.dodge_chance += item.statValue * 0.5
        if (item.stat === 'luck') this.player.crit_chance += item.statValue * 1
      })
    }
  }
  startQuest(dur) {
  ...
  startTavern(durationMinutes) {
    ...
    if ((this.player.provisions || 0) < neededProv) {
      alert('Nicht genug Proviant! BenÃ¶tigt: ' + neededProv + ', Vorhanden: ' + (this.player.provisions || 0))
      return
    }
  ...
  fight() {
    ...
    const step = () => {
      ...
      if (!CombatSystem.dodge(this.monster)) {
        ...
        this.log.push('Du triffst fÃ¼r ' + d + (c ? ' (KRIT!)' : ''))
      ...
      if (!CombatSystem.dodge(this.player)) {
        ...
        this.log.push('Monster trifft fÃ¼r ' + d + (c ? ' (KRIT!)' : ''))
...
function showResult(scene, won, gold, xp, drop) {
  ...
  setTimeout(() => {
    ...
    btn.on('pointerdown', () => {
      ...
      scene.scene.restart({ player: scene.player })
    })
    scene.ui.add(btn)
  }, 1000)
}
...
class LoginScene extends Phaser.Scene {
  ...
  create() {
    ...
    if (loggedIn) {
      this.scene.start('CharacterSelect', { username: loggedIn })
      return
    }
    ...
    // Name input
    this.add.text(w/2, 220, 'Name:', { fontSize: '18px', color: '#fff' }).setOrigin(0.5)
    const nameBtn = this.add.text(w/2, 250, '[ Name eingeben ]', { fontSize: '18px', color: '#00ff00', backgroundColor: '#003300', padding: {x:15,y:8} }).setOrigin(0.5).setInteractive()
    let charName = ''
    nameBtn.on('pointerdown', () => {
      const n = prompt('Name eingeben:')
      if (n && n.trim()) { charName = n.trim(); nameBtn.setText('Name: ' + charName) }
    })
    ...
    loginBtn.on('pointerdown', () => {
      if (!charName || !password) { alert('Name und Passwort erforderlich!'); return }
      const result = login(charName, password)
      if (result.error) { alert(result.error); return }
      this.scene.start('CharacterSelect', { username: charName })
    })
    ...
    regBtn.on('pointerdown', () => {
      if (!charName || !password) { alert('Name und Passwort erforderlich!'); return }
      const result = createAccount(charName, password)
      if (result.error) { alert(result.error); return }
...
class CharacterCreation extends Phaser.Scene {
  ...
  async create() {
    ...
    let charName = ''
    const nameBtn = this.add.text(w/2, 150, 'Name wÃ¤hlen (Klick)', { fontSize: '20px', color: '#00ff00', backgroundColor: '#003300', padding: {x:15,y:8} }).setOrigin(0.5).setInteractive()
    nameBtn.on('pointerdown', () => { const n = prompt('Charaktername:'); if (n && n.trim()) { charName = n.trim(); nameBtn.setText('Name: ' + charName) } })
    
    this.add.text(w/2, 200, 'Klasse wÃ¤hlen:', { fontSize: '22px', color: '#fff', fontStyle: 'bold' }).setOrigin(0.5)
    let idx = 0
    ...
    const next = this.add.text(w/2+250, 180, 'NÃ¤chste >', { fontSize: '18px', color: '#00aaff', backgroundColor: '#002233', padding: {x:15,y:8} }).setOrigin(0.5).setInteractive()
    ...
    createBtn.on('pointerdown', async () => {
      if (!charName) { alert('WÃ¤hle erst einen Namen!'); return }
      try {
        ...
        cls.stats.forEach(s => {
          if (s === 'StÃ¤rke') stats.strength += 15
          if (s === 'Intelligenz') stats.intelligence += 15
          if (s === 'Geschicklichkeit') stats.dexterity += 15
          if (s === 'VitalitÃ¤t') stats.vitality += 15
          if (s === 'GlÃ¼ck') stats.luck += 15
...
class GameScene extends Phaser.Scene {
  ...
  async create() {
    ...
    // Lade Charakter vom Backend falls nicht Ã¼bergeben
    if (!this.player || !this.player.id) {
      const user = getLoggedInUser()
      if (user) {
        const chars = await loadAccountCharacters()
        if (chars && chars.length > 0) {
          this.player = chars[0] // Ersten Charakter laden
        }
      }
    ...
    const menus = [
      ...
      { text:'Inventar & AusrÃ¼stung', scene:'InventoryScene', color:'#aa00ff', bg:'#330055' },
      ...
      { text:'TÃ¤gliche Quests', scene:'DailyQuestScene', color:'#00ffff', bg:'#003333' },
    ...
    const backBtn = this.add.text(w/2, h-40, '[ Charakter wÃ¤hlen ]', { fontSize:'16px', color:'#aaa', backgroundColor:'#333333', padding:{x:15,y:8} }).setOrigin(0.5).setInteractive()
  ...
  testPlayer() {
    this.player = { id:1, name:'Test-Held', class_type:'Krieger', level:5, xp:500, gold:1000, provisions: 200,
      ...
      inventory:[genItem(5),genItem(5),genItem(5)], equipment:{}, dungeonProgress:0, pvpMMR:1000, guildId:null,
...
class QuestScene extends Phaser.Scene {
  ...
  create() {
    ...
    const backBtn = this.add.text(w/2, h-40, '[ ZurÃ¼ck ]', { fontSize:'16px', color:'#aaa', backgroundColor:'#333333', padding:{x:15,y:8} }).setOrigin(0.5).setInteractive()
...
class TavernScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(w/2, 110, 'Kosten: ' + costPerHour + ' Proviant/Stunde', { fontSize:'14px', color:'#aaa' }).setOrigin(0.5)
    const quests = [
      { text:'5 Min (GewÃ¶hnlich)', dur:5, color:'#aaaaaa', bg:'#333333' },
      { text:'10 Min (UngewÃ¶hnlich)', dur:10, color:'#00aaff', bg:'#002233' },
      { text:'15 Min (Selten)', dur:15, color:'#aa00ff', bg:'#330055' },
      { text:'20 Min (LegendÃ¤r)', dur:20, color:'#ffaa00', bg:'#553300' },
      { text:'25 Min (Episch)', dur:25, color:'#ff5500', bg:'#552200' },
      { text:'30 Min (LegendÃ¤r)', dur:30, color:'#ff0000', bg:'#330000' }
    ]
    this.add.text(w/2, 150, 'WÃ¤hle Quest-Dauer:', { fontSize:'16px', color:'#fff' }).setOrigin(0.5)
    quests.forEach((q, i) => {
...
class DungeonScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(w/2, 120, 'NÃ¤chster Boss: Level ' + ((this.player.dungeonProgress || 0) + 1), { fontSize:'18px', color:'#ff6600' }).setOrigin(0.5)
    this.loop = new GameLoop(this); this.loop.setPlayer(this.player)
    const fightBtn = this.add.text(w/2, 180, '[ Boss bekÃ¤mpfen ]', { fontSize:'22px', color:'#ff0000', backgroundColor:'#330000', padding:{x:20,y:10} }).setOrigin(0.5).setInteractive()
...
class InventoryScene extends Phaser.Scene {
  ...
  async create() {
    ...
    this.add.text(w/2, 30, 'Inventar & AusrÃ¼stung', { fontSize:'28px', color:'#aa00ff', fontStyle:'bold', stroke:'#000', strokeThickness:4 }).setOrigin(0.5)
    ...
    // Equipment
    this.add.text(w/2, 100, 'AusrÃ¼stung:', { fontSize:'16px', color:'#fff' }).setOrigin(0.5)
    let y = 120
  ...
  async equipItem(item) {
    try {
      ...
      alert('Fehler beim AusrÃ¼sten: ' + error.message)
    }
  }
}
  ...
  async create() {
    ...
    try {
      ...
      const categories = [
        ...
        { name:'RÃ¼stungen', slot:'armor', color:'#00aaff' },
...
class PvPScene extends Phaser.Scene {
  ...
  showResult(won, gold, xp, drop) {
    ...
    setTimeout(() => {
      ...
      if (won) this.ui.add(this.add.text(w/2, 520, 'MMR: ' + (mmrChange>0?'+':'') + mmrChange + ' (Jetzt: ' + this.player.pvpMMR + ')', { fontSize:'20px', color:'#ffd700' }).setOrigin(0.5))
      const btn = this.add.text(w/2, 560, '[ Beenden ]', { fontSize:'18px', color:'#fff', backgroundColor:'#333333', padding: {x:15,y:8} }).setOrigin(0.5).setInteractive()
      btn.on('pointerdown', () => { if (this.ui) { this.ui.destroy(); this.ui=null }; this.scene.start('PvPScene', { player: this.player, username: this.username }) })
      if (this.ui) this.ui.add(btn)
      saveCharacterToAccount(this.player)
    }, 1000)
  }
  ...
  async create() {
    ...
    try {
      ...
      if (guilds && guilds.length > 0) {
        this.add.text(w/2, 80, 'VerfÃ¼gbare Gilden:', { fontSize:'18px', color:'#ffff00' }).setOrigin(0.5)
        let y = 120
    ...
    // Neue Gilde grÃ¼nden
    const createBtn = this.add.text(w/2, 300, '[ Gilde grÃ¼nden (1000 Gold) ]', { fontSize:'20px', color:'#ffff00', backgroundColor:'#444400', padding:{x:20,y:10} }).setOrigin(0.5).setInteractive()
    createBtn.on('pointerdown', async () => {
...
class DailyQuestScene extends Phaser.Scene {
  ...
  async create() {
    const w = this.cameras.main.width, h = this.cameras.main.height
    this.cameras.main.setBackgroundColor('#0a2a2a')
    this.add.text(w/2, 30, 'TÃ¤gliche Quests (5/Tag)', { fontSize:'28px', color:'#00ffff', fontStyle:'bold', stroke:'#000', strokeThickness:4 }).setOrigin(0.5)
    
    try {
      // Lade tÃ¤gliche Quests vom Backend
      ...
      let y = 120
      quests.forEach(q => {
        const rarityName = q.rarity === 'Common' ? 'GewÃ¶hnlich' : q.rarity === 'Rare' ? 'Selten' : 'Episch'
  ...
  async showResult(won, gold, xp, drop) { 
    ...
    this.add.text(w/2, 80, 'Verbleibend: ' + this.player.dailyQuests.filter(q=>!q.completed).length + '/5', { fontSize:'16px', color:'#fff' }).setOrigin(0.5)
    let y = 120
    this.player.dailyQuests.forEach(q => {
      const txt = RARITIES[q.rarity].name + ' Quest (' + q.duration + ' Min)' + (q.completed?' - ERLEDIGT':'')
      const color = q.completed?'#888':'#00ffff', bg = q.completed?'#222':'#002222'
      const btn = this.add.text(w/2, y, txt, { fontSize:'16px', color:color, backgroundColor:bg, padding:{x:15,y:8} }).setOrigin(0.5).setInteractive()
      if (!q.completed) btn.on('pointerdown', () => { if (!this.loop||!this.loop.active) { q.completed=true; this.loop=new GameLoop(this); this.loop.setPlayer(this.player); this.loop.startDaily(q) } })
      y += 40
    })
    const backBtn = this.add.text(w/2, h-40, '[ ZurÃ¼ck ]', { fontSize:'16px', color:'#aaa', backgroundColor:'#333333', padding:{x:15,y:8} }).setOrigin(0.5).setInteractive()
    backBtn.on('pointerdown', () => { saveCharacterToAccount(this.player); this.scene.start('GameScene', { player: this.player, username: this.username }) })
  }
  showStart(monster, rarity) { createCombatUI(this, this.cameras.main.width, this.cameras.main.height, monster) }
  updateBars(pHP, mHP) { updateBars(this, pHP, mHP) }
  updateLog(log) { updateLog(this, log) }
...
class HideoutScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(w/2, 110, 'Einkommen: +' + ((this.player.hideoutLevel || 0) * 10) + ' Proviant/Tag', { fontSize:'16px', color:'#ffaa00' }).setOrigin(0.5)
    this.add.text(w/2, 140, 'Gold: ' + this.player.gold, { fontSize:'16px', color:'#ffd700' }).setOrigin(0.5)
    const upgradeCost = ((this.player.hideoutLevel || 0) + 1) * 500
    const upgradeBtn = this.add.text(w/2, 200, '[ Hideout ausbauen (' + upgradeCost + ' Gold) ]', { fontSize:'20px', color:'#ffaa55', backgroundColor:'#553311', padding:{x:20,y:10} }).setOrigin(0.5).setInteractive()
    upgradeBtn.on('pointerdown', () => {
      if (this.player.gold >= upgradeCost) {
        this.player.gold -= upgradeCost
        this.player.hideoutLevel = (this.player.hideoutLevel || 0) + 1
        this.player.hideoutIncome = this.player.hideoutLevel * 10
        saveCharacterToAccount(this.player)
        this.scene.restart({ player: this.player, username: this.username })
      } else { alert('Nicht genug Gold! BenÃ¶tigt: ' + upgradeCost) }
    })
    const collectBtn = this.add.text(w/2, 260, '[ Proviant sammeln ]', { fontSize:'20px', color:'#ffaa55', backgroundColor:'#553311', padding:{x:20,y:10} }).setOrigin(0.5).setInteractive()
    collectBtn.on('pointerdown', () => {
      const income = (this.player.hideoutLevel || 0) * 10
      this.player.provisions = (this.player.provisions || 0) + income
      alert('Gesammelt: +' + income + ' Proviant')
      saveCharacterToAccount(this.player)
      this.scene.restart({ player: this.player, username: this.username })
    })
    const backBtn = this.add.text(w/2, h-40, '[ ZurÃ¼ck ]', { fontSize:'16px', color:'#aaa', backgroundColor:'#333333', padding:{x:15,y:8} }).setOrigin(0.5).setInteractive()
