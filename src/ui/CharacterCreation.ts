...
// Klassen-Definitionen (ohne TypeScript-Interface)
const CLASSES = [
  // Reine Klassen
...
export class CharacterCreation extends Phaser.Scene {
  ...
  create() {
    ...
    // Titel mit Schatten
    const title = this.add.text(this.cameras.main.centerX, 60, 'Charakter erstellen', { 
      fontSize: '36px', 
      color: '#ffd700',
      fontStyle: 'bold',
      stroke: '#000',
    ...
    // Name-Eingabe Button (besseres Design)
    const nameButton = this.add.text(this.cameras.main.centerX, 150, 'Name wählen: (Klick hier)', { 
      fontSize: '20px', 
      color: '#00ff00',
    ...
    nameButton.on('pointerdown', () => {
      const name = prompt('Gib deinen Charakternamen ein:')
      if (name && name.trim()) {
    ...
    // Klassen-Anzeige (besseres Design)
    var selectedClassIndex = 0
    const classText = this.add.text(this.cameras.main.centerX, 240, '', { 
      fontSize: '24px', 
      color: '#ffd700',
      fontStyle: 'bold',
      stroke: '#000',
    ...
    const descText = this.add.text(this.cameras.main.centerX, 280, '', { 
      fontSize: '16px', 
      color: '#ccc' 
    }).setOrigin(0.5)
    
    const statsText = this.add.text(this.cameras.main.centerX, 320, '', { 
      fontSize: '16px', 
      color: '#00ff00' 
    }).setOrigin(0.5)
    
    const updateClassDisplay = () => {
      const classInfo = CLASSES[selectedClassIndex]
    ...
    // Vorherige Klasse (besseres Design)
    const prevButton = this.add.text(this.cameras.main.centerX - 100, 240, '< Vorherige', { 
      fontSize: '18px', 
      color: '#00aaff',
    ...
    // Nächste Klasse (besseres Design)
    const nextButton = this.add.text(this.cameras.main.centerX + 100, 240, 'Nächste >', { 
      fontSize: '18px', 
      color: '#00aaff',
      backgroundColor: '#002233',
      padding: { x: 10, y: 5 }
    ...
    // Charakter erstellen Button (besseres Design)
    const createButton = this.add.text(this.cameras.main.centerX, 400, 'Charakter erstellen', { 
      fontSize: '24px', 
      color: '#fff', 
